...
def map_other_data_array3D(df: pd.DataFrame) -> tuple:
    ...
    BUCKET_NAME = "brain-mnist"
    np.save(f'data/{dataset_name}_filtered_{detail}_X.npy', X, allow_pickle=True, fix_imports=True) #save X locally
    np.save(f'data/{dataset_name}_filtered_{detail}_y.npy', y, allow_pickle=True, fix_imports=True) #save y locally
    upload_blob(BUCKET_NAME, f'data/{dataset_name}_filtered_{detail}_X.npy', f"other_datasets/{dataset_name}_filtered_{detail}_X.npy")
    upload_blob(BUCKET_NAME, f'data/{dataset_name}_filtered_{detail}_y.npy', f"other_datasets/{dataset_name}_filtered_{detail}_y.npy")
