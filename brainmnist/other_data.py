...
def map_other_data_array3D(df: pd.DataFrame) -> tuple:
    ...
    BUCKET_NAME = "brain-mnist"
    np.save(f'data/{dataset_name}_filtered_cut_128Hz_X.npy', X, allow_pickle=True, fix_imports=True) #save X locally
    np.save(f'data/{dataset_name}_filtered_cut_128Hz_y.npy', y, allow_pickle=True, fix_imports=True) #save y locally
    upload_blob(BUCKET_NAME, f'data/{dataset_name}_filtered_cut_128Hz_X.npy', f"other_datasets/{dataset_name}_filtered_cut_128Hz_X.npy")
    upload_blob(BUCKET_NAME, f'data/{dataset_name}_filtered_cut_128Hz_y.npy', f"other_datasets/{dataset_name}_filtered_cut_128Hz_y.npy")
